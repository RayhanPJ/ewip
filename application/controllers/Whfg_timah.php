...
class Whfg_timah extends CI_Controller {
    ...
    public function pre_print_receipt($id)
    {
        $data = array(
            'data' => $this->ProdControl->getDataScan($id),
            'item' => $this->M_Line->getData(),
        );
        $this->load->view('template/header');
        $this->load->view('whfg_timah/form_list_item_timah', $data);
        $this->load->view('template/footer');
    }
    ...
    public function resetData($id)
    {
        ...
        redirect(base_url('whfg_timah/list_receipt/'));
    }

    public function print_receipt($barc)
    {
        $data = array(
            'data' => $this->M_Line->getDataById($barc),
        );
        // $this->load->view('template/header');
    ...
    {
        $data = array(
            'data' => $this->M_Line->getDataById($id),
        );
    ...
    public function getDataDnAdj($id='')
    {
        ...
        $data['data'] = $this->ProdControl->getLineDn(strtoupper($noDn));
        $data['data2'] = $this->ProdControl->getLineDnSummary(strtoupper($noDn));
        $data['data3'] = $this->M_Line->getData();
        $last_data = $this->M_Line->getLastData();
        $data['last_line'] = $last_data[0]['line'];
        $data['last_locations'] = $last_data[0]['locations'];
        $data['last_sub_locations'] = $last_data[0]['sub_locations'];
        // var_dump($data['data']);die();
    ...
    public function addDataTimahAdj()
    {
        ...
        $dn         = $this->ProdControl->getLineDnDetail($nodn);
        // var_dump($line_dn);die();

        foreach ($dn as $d) {

            if ($d->BP == 'SPL000010') {
                $cwar = 'K-NFU';
            } elseif ($d->BP == 'SPL000180') {
                $cwar = 'K-IML';
            } elseif ($d->BP == 'SPL000056') {
                $cwar = 'K-KRB';
            } elseif ($d->BP == '') {
                $cwar = 'K-EMA';
            }
        ...
        if ($updateStockLead == TRUE) {
            ...
            foreach ($dn as $d) {
                ...
                $insertResult = $this->Sqlmodel->insert_data('data_whfg_timah', $data);

                /* Input data to local DB (rayhan)*/
                $data2 = [
                    // 'line' => $this->input->post('line'),
                    'barc' => $barc,
                    'created_at_storing' => date('Y-m-d H:i:s.u'),
                ];

                $updateLocalResult = $this->M_Line->update_data('store_timah', $data2, $id);
                // var_dump($result);die();
            }

            if ($insertResult > 0 && $updateLocalResult > 0) {
                $redirectPath = 'whfg_timah/print_receipt_adj/' . $barc;
            } else {
                $redirectPath = 'whfg_timah/print_receipt_adj/' . $barc;
            }
        } else {
