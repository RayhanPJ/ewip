        ...
        <div class="block">
            <div class="block-content block-content-full">
                ...
                <?php
                    ...
                    <hr>    
                    <form action="<?=base_url()?>order_plate/confirmSupply" method="POST">
                        <table class="table-supply-plate" style="margin: 0 auto;">
                            <tr>
                                <td>Jenis Plate</td>
                                <td> : </td>
                                <td>
                                    <?=$data[$count]['jenis_plate']?>
                                    <input type="hidden" name="line" id="line" value="<?=$data[$count]['line']?>">
                                </td>
                            </tr>
                            <tr>
                                <td>Type Plate</td>
                                <td> : </td>
                                <td>
                                    <?=$data[$count]['type_plate']?>
                                    <input type="hidden" name="type_plate" id="type_plate" value="<?=$data[$count]['type_plate']?>">
                                </td>
                            </tr>
                            <tr>
                                <td>Jumlah Pnl</td>
                            ...
                            <tr>
                                <td>Jumlah Rak/Basket</td>
                                <td> : </td>
                                <td><?=$data[$count]['jumlah_basket']?></td>
                            </tr>
                            <tr>
                                <td>Create Order</td>
                                <td> : </td>
                            ...
                            <tr>
                                ...
                                <td id="barcodeParent">
                                    ...
                                    <?php 
                                        $data_detail = $this->OrderPlateModel->getDetailOrder($data[$count]['id_order_plate']);
                                        ...
                                        // for ($i=0; $i < $data[$count]['jumlah_basket']; $i++) { 
                                        $count_barcode = (!empty($data_detail)) ? count($data_detail) : $data[$count]['jumlah_basket'];
                                        for ($i=0; $i < $count_barcode; $i++) {
                                            $barc = str_replace(',', '.', $barcode[$i]['barcode']); 
                                            ?>
                                            <div class="input-group" style="margin-bottom: 10px;" id="barcode_group_<?=$i?>">
                                                <input type="text" class="form-control supply-input" id="barcode_<?=$i?>" name="barcode[]" onchange="scanQr(<?=$i?>)" value="<?=$barc?>" minlength="40" maxlength="40" style="width:600px;">
                                                <div class="input-group-append">
                                                    <button type="button" class="btn btn-danger" onclick="delete_barcode(<?=$i?>)" <?=$disabled?>><i class="fa fa-trash"></i></button>
                                                </div>
                                            </div>
                                        <?php } ?>
                                ...
                                <td id="supplyParent">
                                    ...
                                    <?php 
                                        ...
                                        if (!empty($data_detail)) {
                                            ...
                                            $qty_supply[0]['qty_order_plate'] = '';
                                            $qty_supply[1]['qty_order_plate'] = '';
                                            $qty_supply[2]['qty_order_plate'] = '';
                                            $qty_supply[3]['qty_order_plate'] = '';
                                            $qty_supply[4]['qty_order_plate'] = '';
                                            $qty_supply[5]['qty_order_plate'] = '';
                                            $qty_supply[6]['qty_order_plate'] = '';
                                            $qty_supply[7]['qty_order_plate'] = '';
                                            $qty_supply[8]['qty_order_plate'] = '';
                                            $qty_supply[9]['qty_order_plate'] = '';
                                            $qty_supply[10]['qty_order_plate'] = '';
                                        }

                                        // for ($i=0; $i < $data[$count]['jumlah_basket']; $i++) { 
                                        $count_qty = (!empty($data_detail)) ? count($data_detail) : $data[$count]['jumlah_basket'];
                                        for ($i=0; $i < $count_qty; $i++) { 
                                            $qty_actq = str_replace(',', '.', $qty_supply[$i]['qty_order_plate']); 
                                            ?>
                                            <div class="input-group" style="margin-bottom: 10px;" id="supply_group_<?=$i?>">
                            ...
                            <tr>
                                ...
                                <td> : </td>
                                <td>
                                    <select name="keterangan" id="keterangan" class="form-control input-group">
                                        <option value="" selected disabled>- Pilih jika barcode/qty 0 -</option>
                                        <!-- <option value="Ok">Ok</option> -->
                                        <option value="Unform/Form plate kosong/habis" <?= ($data[$count]['keterangan'] == 'Unform/Form plate kosong/habis') ? 'selected' : '' ?>>Unform/Form plate kosong/habis</option>
                                        <option value="Quantity tidak lotsize" <?= ($data[$count]['keterangan'] == 'Quantity tidak lotsize') ? 'selected' : '' ?>>Quantity tidak lotsize</option>
                                        <option value="Salah order" <?= ($data[$count]['keterangan'] == 'Salah order') ? 'selected' : '' ?>>Salah order</option>
                                        <option value="Problem scan karena Jaringan" <?= ($data[$count]['keterangan'] == 'Problem scan karena Jaringan') ? 'selected' : '' ?>>Problem scan karena Jaringan</option>
                                        <option value="Problem scan karena Blocked" <?= ($data[$count]['keterangan'] == 'Problem scan karena Blocked') ? 'selected' : '' ?>>Problem scan karena Blocked</option>
                                        <option value="Label hanya satu" <?= ($data[$count]['keterangan'] == 'Label hanya satu') ? 'selected' : '' ?>>Label hanya satu</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <?php if($data[$count]['jenis_plate'] == 'Unform') { ?>
                                    <td></td>
                                    <td></td>
                                    <td></td>
                                <?php } else { ?>
                                    ...
                                    <td> : </td>
                                    <td>
                                        <select name="supply_by" id="supply_by" class="form-control input-group" required>
                                            <option value="" selected disabled>- Pilih metode supply -</option>
                                            <option value="Forklift" <?= ($data[$count]['supply_by'] == 'Forklift') ? 'selected' : '' ?>>Forklift</option>
                                            <option value="AGV" <?= ($data[$count]['supply_by'] == 'AGV') ? 'selected' : '' ?>>AGV</option>
                                        </select>
                                    </td>
                                <?php } ?>
                            </tr>
                        ...
                        <input type="text" name="id_order_plate" value="<?=$data[$count]['id_order_plate']?>" readonly>
                    </form>
                    <?php } ?>     
                </div>
            </div>
...
<script>
    ...
    function filterByDate() {
        ...
        window.location.replace('<?=base_url(); ?>order_plate/activity_supply_assy_a/'+tanggal);
    }

    function filterByDelivery() {
        var tanggal = $('#filterTanggal').val()
        var sesi = $('#filter_delivery').val()
        window.location.replace('<?=base_url(); ?>order_plate/activity_supply_assy_a/'+tanggal+'/'+sesi);
    }

    function linkPage(i) {
        var tanggal = $('#filterTanggal').val()
        var sesi = $('#filter_delivery').val()
        window.location.replace('<?=base_url(); ?>order_plate/activity_supply_assy_a/'+tanggal+'/'+sesi+'/'+i);
    }

    function scanQr(index) {
        console.log($("#barcode_"+index).val());
        let barc = $("#barcode_"+index).val();
        let type_plate = $("#type_plate").val().substr(0, 4);
        ...
        document.addEventListener('keyup', function(event) {
            if (event.keyCode == 9) {
                if(arrQr.includes(barc) == true) {
                ...
                } else if (!barc.includes(type_plate)) {
                    window.alert('Barcode tidak sesuai type plate yang di order');
                    $("#barcode_"+index).val('');
                    $('#loading-modal').modal('hide');
                    $("#barcode_"+index).focus();
                } else if (barc.length < 40 && barc.length > 40) {
                    window.alert('Panjang Barcode tidak sesuai standar');
                    $("#barcode_"+index).val('');
                    $('#loading-modal').modal('hide');
                    $("#barcode_"+index).focus();
                } else {
                    if(alertshown == false) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?=base_url(); ?>order_plate/checkQtyBarcode",
                            async: true,
                            dataType: 'json',
                            data:{
                                barc : barc
    ...
    function add_barcode() {
        ...
        newBarcodeInput.id = 'barcode_group_' + totalInputs;
        newBarcodeInput.innerHTML = '<input type="text" class="form-control supply-input" id="barcode_' + totalInputs + '" name="barcode[]" onchange="scanQr(' + totalInputs + ')" value="" minlength="40" maxlength="40" style="width:600px;"><div class="input-group-append"><button type="button" class="btn btn-danger" onclick="delete_barcode(' + totalInputs + ')"><i class="fa fa-trash"></i></button></div>';
        ...
        newSupplyInput.id = 'supply_group_' + totalInputs;
        newSupplyInput.innerHTML = '<input type="number" class="form-control supply-input" id="actual_supply_' + totalInputs + '" name="actual_supply[]" value="" required><div class="input-group-append"><span class="input-group-text">pnl</span></div>';
    ...
    function delete_barcode(index) {
        if (window.confirm('Apakah anda yakin menghapus data ini?')) {
            $('#barcode_group_' + index).remove();
            $('#supply_group_' + index).remove();
        }
    }
</script>